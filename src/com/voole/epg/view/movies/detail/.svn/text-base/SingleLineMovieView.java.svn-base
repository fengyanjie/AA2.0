...
import com.voole.epg.view.movies.movie.MovieViewFocusListener;
...
//	public static final int ITEM_SIZE = 10;
	...
	public SingleLineMovieView(Context context, AttributeSet attrs) {
		...
		ITEM_SIZE = itemSize;
		isDisplayArrow = a.getBoolean(R.styleable.MovieView_displayArrow, true);
		textSize= a.getInt(R.styleable.MovieView_textSize, 16);
		imgWeight= a.getInt(R.styleable.MovieView_imgWeight, 0);
		textWeight= a.getInt(R.styleable.MovieView_textWeight, 0);
		a.recycle();
		init(context);
	}
	...
	private void initMovies(Context context){
		...
		param_arrow_left.leftMargin = DisplayManager.GetInstance().changeWidthSize(20);
		...
		layout_middle_item_param.setMargins(5, 0, 5, 0);
		...
		param_arrow_right.rightMargin = DisplayManager.GetInstance().changeWidthSize(45);
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		switch (keyCode) {
			...
			case KeyEvent.KEYCODE_DPAD_RIGHT:
				if(films != null && selectedItemIndex < films.size() - 1 && selectedItemIndex < ITEM_SIZE){
					itemViews[selectedItemIndex].setFocusedItem(false);
					itemViews[++selectedItemIndex].setFocusedItem(true);
					if (movieViewFocusListener!=null) {
						movieViewFocusListener.onItemFocusChange(selectedItemIndex);
