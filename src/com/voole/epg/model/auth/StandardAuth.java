package com.voole.epg.model.auth;
...
public class StandardAuth implements IAuth{
	...
	public void startAuth() {
		...
		String rtConfig = getDir() + "/" + AUTH_CONF_RT_NAME;
		...
		LogUtil.d("modulepath--->" + modulepath);
		if(!isAuthRunning()){
	...
	public void exitAuth() {
		...
		VooleGLib.killExe(AUTH_FILE_NAME);
	}
...
//		}
	...
	public boolean isAuthRunning() {
		int status = VooleGLib.isExeRunning(AUTH_FILE_NAME);
		String modulepath = getDir() + "/" + AUTH_FILE_NAME;
		File auth = new File(modulepath);
		if(status < 0){
